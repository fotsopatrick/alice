...
PARAM_PROVIDER = "tour_community_braignak.provider"  # "deepseek" (défaut) | "gemini"

_ENDPOINT = {
    "deepseek": "https://api.deepseek.com/chat/completions",
    "gemini": "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",
}
_MODELE_DEFAUT = {"deepseek": "deepseek-chat", "gemini": "gemini-3.5-flash"}
...
class BraignakCommunity(http.Controller):
    ...
    def observer(self, cible):
        ...
        fournisseur = (icp.get_param(PARAM_PROVIDER) or "").strip().lower()
        if not fournisseur:
            fournisseur = "deepseek" if cle.startswith("sk-") else "gemini"
        if fournisseur not in _ENDPOINT:
            fournisseur = "deepseek"
        modele = (icp.get_param(PARAM_MODELE) or "").strip() or \
            _MODELE_DEFAUT.get(fournisseur, "deepseek-chat")
