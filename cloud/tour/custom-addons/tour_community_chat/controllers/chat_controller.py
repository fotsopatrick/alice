...
2. Sinon (installation locale sans le pont), elle appelle l'API compatible
   OpenAI DIRECTEMENT avec la clé que l'administrateur a posée — DeepSeek
   (sk-…) ou Google Gemini (AIza…/AQ.…, fournisseur 'gemini', modèle par
   défaut gemini-3.5-flash) — et écrit l'app dans le dossier servi. C'est le
   mode « utilisateur lambda » : une clé suffit.
...
PARAM_PROVIDER = "tour_community_chat.provider"  # "deepseek" (défaut) | "gemini"
...
# Dossier où Chloé écrit les apps. Sur Cloud Run, /var/lib/odoo est en lecture
# seule : pointer TOUR_APPS_DIR vers /tmp/community-apps.
DOSSIER_APPS = os.environ.get("TOUR_APPS_DIR",
                              "/var/lib/odoo/community-apps")

ENDPOINTS = {
    "deepseek": ("https://api.deepseek.com/chat/completions", "deepseek-chat", 3000),
    "gemini": ("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions", "gemini-3.5-flash", 12288),
}
...
class ChloéCommunity(http.Controller):
    ...
    def enregistrer_cle(self, cle=None):
        ...
        maille = re.match(r"^(sk-|AIza[A-Za-z0-9_-]{20,}|AQ\.[A-Za-z0-9_-]+)$", cle)
        if not maille:
            return {"erreur": "Cette clé ne ressemble ni à une clé DeepSeek (sk-…) ni à une clé Google AI Studio (AIza…/AQ.…)."}
        fournisseur = "deepseek" if cle.startswith("sk-") else "gemini"
        icp = request.env["ir.config_parameter"].sudo()
        icp.set_param(PARAM_CLE, cle)
        icp.set_param(PARAM_PROVIDER, fournisseur)
        return {"ok": True, "fournisseur": fournisseur}

    def _cle_api(self):
        """La clé du fournisseur : d'abord dans le Coffre, puis le paramètre."""
        if "vault.secret" in request.env:
            cle = request.env["vault.secret"].sudo()._lire(
                CLE_COFFRE, "chat Community")
            if cle:
                return cle
        return (request.env["ir.config_parameter"].sudo()
                .get_param(PARAM_CLE) or "").strip()

    def _fournisseur(self, cle):
        """Détecte le fournisseur à partir de la clé si le paramètre est vide."""
        icp = request.env["ir.config_parameter"].sudo()
        prov = (icp.get_param(PARAM_PROVIDER) or "").strip().lower()
        if not prov:
            prov = "deepseek" if cle.startswith("sk-") else "gemini"
        return prov
    ...
    def message(self, texte, historique=None):
        ...
        # 2) Repli local : DeepSeek (ou Gemini) direct avec la clé de l'instance.
        cle = self._cle_api()
        if not cle:
            return {"erreur": (
        ...
        fournisseur = self._fournisseur(cle)
        return self._chat_local(cle, fournisseur, fil, texte, invite)
    ...
    # --- repli local (API compatible OpenAI : DeepSeek ou Gemini) --------

    def _chat_local(self, cle, fournisseur, fil, texte, invite):
        ...
        endpoint, modele_defaut, max_tokens = ENDPOINTS.get(
            fournisseur, ENDPOINTS["deepseek"])
        icp = request.env["ir.config_parameter"].sudo()
        modele = (icp.get_param(PARAM_MODELE) or "").strip() or modele_defaut
        ...
        for _ in range(4):  # boucle bornee d'outils
            try:
                r = requests.post(
                    ...
                    json={"model": modele, "messages": msgs,
                          "tools": outils_oa, "max_tokens": max_tokens},
                    timeout=120)
            except requests.RequestException:
                return {"erreur": "Impossible de joindre l'API — réseau du serveur."}
            if r.status_code == 401:
                return {"erreur": "Clé API invalide ou expirée — vérifie %s." % PARAM_CLE}
            if r.status_code == 429:
                return {"erreur": "Limite de débit API atteinte — réessaie dans un instant."}
            if r.status_code >= 400:
                _logger.warning("Chat Community : API %s : %s", r.status_code, r.text[:200])
                return {"erreur": "Erreur API (%s)." % r.status_code}
            data = r.json()
